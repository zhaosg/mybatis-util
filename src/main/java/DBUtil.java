...
public class DBUtil {
    ...
    public static Connection openConnection(String configUrl) throws SQLException, ClassNotFoundException, IOException {
        if (null == con || con.isClosed()) {
            Properties p = new Properties();
            p.load(DBUtil.class.getResourceAsStream(configUrl));
            Class.forName(p.getProperty("jdbc.driver"));
            con = DriverManager.getConnection(p.getProperty("jdbc.read.url"), p.getProperty("jdbc.read.username"),
                    p.getProperty("jdbc.read.password"));
        }
        return con;
    }
